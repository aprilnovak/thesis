...
\noindent where \(\mathscr{K}_{SF}\) is given\hspace{0.02cm}\footnote{A typo is unfortunately present in Eq.\ \eqref{eq:ZBSKSF} in the author's 2019 journal article \cite{novak_sana}.} as
...
The conduction equation based on the fluctuating heat source is referred to as the \mbox{``microscale''} model,
...
An example in Section \ref{sec:verification_meso} provides a more intuitive grasp of the method. The \gls{hsd} method is considered here due to successful application to cylindrical \gls{cfp} compacts that share many similarities to \gls{pbr} fuels \cite{stainsby}.

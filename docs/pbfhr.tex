...
\begin{table}[htb!]
\caption{Properties and kernel geometry for the \gls{pbfhr} fuel pebble \cite{novak_manual_2020}.}
\centering
\centerline{
...
\textbf{Gap (\si{\milli\meter})} & {\boldmath\(A_{rr}\)} & {\boldmath\(B_{rr}\)} & \textbf{Avg.} & \textbf{Max.} && {\boldmath\(A_{zz}\)} & {\boldmath\(B_{zz}\)} & \textbf{Avg.} & \textbf{Max.}\\
\midrule
